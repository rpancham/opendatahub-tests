...
#     if ns.exists:
#         logger.info(f"Cleaning up existing namespace: {ns_name}")
...
#             if elapsed > timeout:
#                 raise TimeoutError(f"Namespace {ns_name} not deleted after {timeout} seconds")
#             time.sleep(2)

#         logger.info(f"Namespace {ns_name} successfully cleaned up")
# @pytest.fixture(scope="class")
# def triton_grpc_serving_runtime_template(admin_client: DynamicClient) -> Generator[Template, None, None]:
#     grpc_template_yaml = TEMPLATE_FILE_PATH.get(Protocols.GRPC)
#     with Template(
