...
from tests.model_serving.model_runtime.triton.constant import (
    ...
    MODEL_PATH_PREFIX,
    TRITON_GRPC_TF_INPUT_QUERY,
    TRITON_REST_TF_INPUT_QUERY,
)

LOGGER = get_logger(name=__name__)

MODEL_NAME = "inception_graphdef"
TF_MODEL_LABEL = "inceptiongraphdef"
MODEL_VERSION = "1"
MODEL_NAME_DICT = {"name": MODEL_NAME}
MODEL_LABEL_DICT = {"name": TF_MODEL_LABEL}
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            {"name": "inceptiongraphdef-raw-rest"},
            {
                **BASE_RAW_DEPLOYMENT_CONFIG,
                **MODEL_LABEL_DICT,
            },
            ...
            BASE_RAW_DEPLOYMENT_CONFIG,
            id="inceptiongraphdef-raw-rest-deployment",
        ),
        pytest.param(
            {"protocol_type": Protocols.GRPC},
            {"name": "inceptiongraphdef-raw-grpc"},
            {
                **BASE_RAW_DEPLOYMENT_CONFIG,
                **MODEL_LABEL_DICT,
            },
            MODEL_STORAGE_URI_DICT,
            BASE_RAW_DEPLOYMENT_CONFIG,
            id="inceptiongraphdef-raw-grpc-deployment",
        ),
        pytest.param(
            {"protocol_type": Protocols.REST},
            {"name": "inceptiongraphdef-serverless-rest"},
            {**BASE_SERVERLESS_DEPLOYMENT_CONFIG, **MODEL_LABEL_DICT},
            MODEL_STORAGE_URI_DICT,
            BASE_SERVERLESS_DEPLOYMENT_CONFIG,
            id="inceptiongraphdef-serverless-rest-deployment",
        ),
        pytest.param(
            {"protocol_type": Protocols.GRPC},
            {"name": "inceptiongraphdef-serverless-grpc"},
            {**BASE_SERVERLESS_DEPLOYMENT_CONFIG, **MODEL_LABEL_DICT},
            MODEL_STORAGE_URI_DICT,
            BASE_SERVERLESS_DEPLOYMENT_CONFIG,
            id="inceptiongraphdef-serverless-grpc-deployment",
        ),
    ],
    indirect=True,
)
class TestinceptiongraphdefModel:
    """
    Test class for inceptiongraphdef inference using Triton on KServe.
    ...
    def test_inceptiongraphdef_inference(
        self,
        ...
        """
        input_query = TRITON_REST_TF_INPUT_QUERY if protocol == Protocols.REST else TRITON_GRPC_TF_INPUT_QUERY
